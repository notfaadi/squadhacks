# Deploy squadhacks.net

Step-by-step guide to deploy the Squad Hacks static site to **squadhacks.net** on Cloudflare Pages, configure DNS, and submit to Google Search Console.

## Prerequisites

- Node.js **≥ 22.12.0**
- Cloudflare account with access to **squadhacks.net** DNS
...
3. Configure build settings:
   - **Project name:** `squadscheats` (existing) or create a new project
...
This runs `wrangler pages deploy dist --project-name=squadhacks` (see `wrangler.toml`).

## 3. Custom domain and DNS

Add **squadhacks.net** as the primary custom domain on the Pages project.
...
2. In **Rules** → **Redirect Rules** (or Bulk Redirects), create:
   - **Source:** `www.squadhacks.net/*`
   - **Target:** `https://squadhacks.net/${1}`
   - **Status:** 301

The deployed `functions/_middleware.js` also enforces apex canonical host, legacy domain redirects (`squadhacks.net`, `.net`, `.com`), and legacy path redirects.
...
- `https://squadhacks.net/`
- `https://squadhacks.net/es/`
- `https://squadhacks.net/squad-hacks/`
- `https://squadhacks.net/squad-aimbot/`
- `https://squadhacks.net/sitemap.xml`
- `https://squadhacks.net/robots.txt`

Verify redirects:

- `http://squadhacks.net` → `https://squadhacks.net` (301)
- `https://www.squadhacks.net` → `https://squadhacks.net` (301)
- Legacy domains (e.g. `squadhacks.net`) → `https://squadhacks.net` (301)
- `/sitemap-index.xml` → `/sitemap.xml` (301)
- Legacy paths (e.g. `/fortnite-hacks/`) → Squad equivalents (301)

## 5. Google Search Console

1. Go to [Google Search Console](https://search.google.com/search-console).
2. **Add property** → choose **Domain** → enter `squadhacks.net`.
...
4. After verification, open **Sitemaps** and submit:
   ...
   https://squadhacks.net/sitemap.xml
   ```
   Remove any legacy submissions (`sitemap-index.xml`, old `squadhacks.net` URLs).
5. Use **URL Inspection** to request indexing for:
   - Homepage (`/`)
   - Pillar page (`/squad-hacks/`)
   - Key landing pages (`/squad-aimbot/`, `/squad-esp/`, `/squad-hacks-2026/`, etc.)
...
- [ ] Custom domain `squadhacks.net` attached and active
- [ ] `www` redirects to apex
- [ ] Legacy domains 301 to `squadhacks.net`
- [ ] Always Use HTTPS enabled
- [ ] `robots.txt` and sitemaps serve from `https://squadhacks.net`
- [ ] Google Search Console domain verified
- [ ] `sitemap.xml` submitted in GSC
- [ ] Homepage and `/squad-hacks/` requested for indexing
