 ...
 *
 * Do not write these into public/_redirects — Cloudflare Workers caps
 * _redirects at 100 dynamic rules and treats every line as dynamic.
 */
...
function rallySlugBlock(src, pageId) {
...
for (const [fromId, toId] of Object.entries(TARGETS)) {
	const fromSlugs = rallySlugBlock(routing, fromId);
	const toSlugs = rallySlugBlock(routing, toId);
...
console.log(
	`Synced ${Object.keys(map).length / 2} cannibal locale redirect pairs (${Object.keys(TARGETS).length} pageIds) → ${path.relative(ROOT, JSON_OUT)}`,
);
