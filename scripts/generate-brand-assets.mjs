...
const sourcePath =
	'C:/Users/Aman/.cursor/projects/c-Users-Aman-squadhacks/assets/c__Users_Aman_AppData_Roaming_Cursor_User_workspaceStorage_5fc01ad59007d6b1ec3564ba48b04eb7_images_image-4be7ee07-5934-4878-8eb6-1fb2f3143ab4.png';
...
async function generateSiteLogo() {
	...
	await writeFile(path.join(imagesDir, 'squad-hacks-logo.png'), logoPng);
	console.log('Wrote public/images/squad-hacks-logo.png (512×512)');

	const logoWebp = await sharp(logoPng).webp({ quality: 90, effort: 6 }).toBuffer();
	await writeFile(path.join(imagesDir, 'squad-hacks-logo.webp'), logoWebp);
	console.log('Wrote public/images/squad-hacks-logo.webp');
}
...
async function generateWebManifest() {
	const manifest = {
		name: 'Squad Hacks',
		short_name: 'Squad Hacks',
		description: 'Undetected squad hacks — ESP, aimbot, radar and for PC',
		start_url: '/',
		display: 'standalone',
