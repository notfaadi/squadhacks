...
const assetsDir =
	'C:/Users/Aman/.cursor/projects/c-Users-Aman-squadhacks/assets';
...
const USER_SCREENSHOTS = [
	{
		...
		file: 'squad-esp-player-tags.webp',
	},
	{
		source:
		...
		file: 'squad-wallhack-skeleton.webp',
	},
	{
		source:
	...
	},
	{
		...
		file: 'squad-aimbot-skeleton.webp',
	},
	{
		source:
...
const REMOVE_PREFIXES = [
	'squad-battle-royale-combat',
	'squad-rally-fight',
	'squad-hacks-aimbot',
	'squad-hacks-cover',
	'squad-hacks-esp-wallhack',
	'squad-hacks-hero',
	'squad-hacks-logo',
	'squad-hacks-package',
	'squad-header-art',
	'squad-loadout-builder',
	'squad-player-esp',
	'squad-scav-run-combat',
	'squad-scav-run-mode',
	'squad-squad-fight',
	'squad-verdansk-map',
];

async function removeOldSquadImages() {
...
async function generateResponsiveVariants() {
	const heroSource = path.join(imagesDir, 'squad-esp-player-tags.webp');
	const heroMeta = await sharp(heroSource).metadata();

	for (const width of HERO_WIDTHS) {
		if (heroMeta.width && width > heroMeta.width) continue;
		const file = `squad-esp-player-tags-${width}w.webp`;
	...
	for (const { file } of USER_SCREENSHOTS) {
		if (file === 'squad-esp-player-tags.webp') continue;
...
async function generateFavicons() {
	const logoPath = path.join(imagesDir, 'squad-hacks-logo.png');
